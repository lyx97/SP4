...
bool CLuaInterface::Init()
{
    ...
    if ((theLuaState) && (theErrorState))
    {
        ...
        // 3. Load lua error script
        luaL_dofile(theErrorState, "Image//DM2240_errorLookup.lua");
    }
...
void CLuaInterface::Run()
{
    ...
    lua_getglobal(theLuaState, "PlayerPos_x");
    int PlayerPos_x = lua_tointeger(theLuaState, -1);

    lua_getglobal(theLuaState, "PlayerPos_z");
    int PlayerPos_z = lua_tointeger(theLuaState, -1);
    ...
    cout << "player Position X : " << PlayerPos_x << endl;
    cout << "player Position Z : " << PlayerPos_z << endl;
}
...
float CLuaInterface::getCharValue(const char* varName)
{
    lua_getglobal(theLuaState, varName);
...
void CLuaInterface::error(const char *errorCode)
{
    ...
    if (errorMsg != NULL)
        ...
        cout << errorCode << " is not valid. \n*** Please contace the developer ***" << endl;
}